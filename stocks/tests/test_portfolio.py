...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "2019-01-01",
            pd.DataFrame(
                {"Close": [50, 100]}, index=[datetime(2018, 1, 1), datetime(2019, 1, 1)]
            ),
            1.0,
        ),
    ],
...
def test_calculate_cagr_happy_path(
    portfolio, symbol, from_date, to_date, price_data, expected_cagr
):
    # Arrange
    ...
    portfolio.data_api.price_history_by_dates.assert_called_once_with(
...
def test_calculate_cagr_no_data(portfolio, symbol, from_date, to_date, price_data):
    ...
    # Act & Assert
    with pytest.raises(
        ValueError, match=f"No data available for {symbol} in the given date range."
    ):
...
def test_calculate_cagr_invalid_cagr(portfolio, symbol, from_date, to_date, price_data):
    ...
    portfolio.data_api = MagicMock()
    portfolio.data_api.price_history_by_dates.return_value = price_data
    portfolio._calculate_cagr = MagicMock(
        side_effect=Exception("Invalid CAGR calculation")
    )
